...
def create_modules(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            if 'in_channels' in module_def.keys():
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            #     x = layer_outputs[-1] + layer_outputs[layer_i]
                ...
                layer_i = int(module_def["from"])
                if not isinstance(self.module_defs[i + layer_i]['remain'], int) or not isinstance(self.module_defs[i - 1]['remain'], int):
                    if isinstance(self.module_defs[i - 1]['remain'], int):
                        index_1 = torch.tensor(list(range(self.module_defs[i - 1]['old_filters']))).cuda()
                    else:
                        index_1 = torch.tensor(self.module_defs[i - 1]['remain']).cuda()
                    if isinstance(self.module_defs[i + layer_i]['remain'], int):
                        index_2 = torch.tensor(list(range(self.module_defs[i + layer_i]['old_filters']))).cuda()
                    else:
                        index_2 = torch.tensor(self.module_defs[i + layer_i]['remain']).cuda()
                    index_all = torch.tensor(module_def['remain']).cuda()
                    tmp = torch.zeros(layer_outputs[-1].size(0), self.module_defs[i - 1]['old_filters'], layer_outputs[-1].size(2), layer_outputs[-1].size(3)).cuda()
                    tmp[:,index_1] = tmp[:,index_1] + layer_outputs[-1]
                    tmp[:,index_2] = tmp[:,index_2] + layer_outputs[layer_i]
                    x = tmp[:,index_all]
                else:
                    x = layer_outputs[-1] + layer_outputs[layer_i]
